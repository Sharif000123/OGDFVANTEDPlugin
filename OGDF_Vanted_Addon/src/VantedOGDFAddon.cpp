// Native subprocess bridge between VANTED and OGDF.
// Reads GraphML from stdin, writes the resulting GraphML to stdout,
// and writes warnings and machine-readable metrics to stderr.
// This simple process protocol avoids platform-specific Java bindings.

// The program supports three request modes:
// 1) Layout: read an input graph from GraphML, apply an OGDF layout algorithm, and return the laid-out graph.
// 2) Generation: create a synthetic graph from request parameters and lay it out with a default algorithm.
// 3) Metrics: compute graph/layout statistics and report them in a machine-readable format on stderr.
...
namespace
{
    ...
    // Request models -------------------------------------------------------------
    // Structs that hold the layout and generator options passed in via GraphML.
    ...
    // Lightweight request-control parsing ---------------------------------------
    // Java side sends control values inside GraphML tags, so these helpers read and normalize them.
    // Reads the complete GraphML request from standard input.
    ...
    // Converts a string to lowercase for case-insensitive protocol matching.
    ...
    // Decodes the XML entities that may occur in GraphML control values.
    ...
    // Reads one GraphML control value directly before GraphIO parses the graph.
    ...
    // Parses an integer value or returns the supplied fallback.
    ...
    // Parses a decimal value or returns the supplied fallback.
    ...
    // Parses common boolean spellings or returns the supplied fallback.
    ...
    // Request decoding and mode selection ---------------------------------------
    // Functions that decide whether the incoming request is a layout, generation, or metrics job.
    // Reads all layout-related options from the GraphML request.
    LayoutRequest parseLayoutRequest(const std::string &graphMl)
    {
        ...
        request.threshold = parseDouble(graphDataValue(graphMl, "k_ogdf_threshold"), request.threshold);
    ...
    // Graph preparation and compatibility checks --------------------------------
    // Testing for safety: OGDF expects valid node sizes and finite coordinates before layout can run safely.
    // Replaces invalid node dimensions with defaults and reports correction counts.
    void ensureNodeSizes(ogdf::GraphAttributes &ga)
    {
        size_t affectedNodes = 0;
        size_t correctedWidths = 0;
        size_t correctedHeights = 0;
        for (ogdf::node v : ga.constGraph().nodes)
        {
            const bool invalidWidth = !std::isfinite(ga.width(v)) || !(ga.width(v) > 0.0);
            const bool invalidHeight = !std::isfinite(ga.height(v)) || !(ga.height(v) > 0.0);
        ...
        if (affectedNodes > 0)
        {
            std::cerr << "Node-size warning: Corrected invalid or missing node dimensions for "
                      << affectedNodes << " node(s): " << correctedWidths
                      << " width value(s) and " << correctedHeights
                      << " height value(s) were set to 30." << std::endl;
        }
    }

    // Removes existing bends before a new layout creates edge geometry.
    ...
    // Checks whether a nontrivial graph contains multiple connected components.
    bool hasDisconnectedComponents(const ogdf::Graph &g)
    {
        return g.numberOfNodes() > 1 && !ogdf::isConnected(g);
    }

    // Reports why an incompatible layout was left unapplied.
    void warnLayoutNotApplied(const std::string &reason)
    {
        std::cerr << "Warning: Layout not applied: " << reason << std::endl;
    }

    // Replaces non-finite coordinates with deterministic grid positions.
    ...
    // Places generated grid-graph nodes in evenly spaced rows and columns.
    ...
    // Layout module factories ----------------------------------------------------
    // These helpers choose the OGDF ranking, crossing-minimization, and layout strategies for each mode.
    // Creates the selected Sugiyama ranking module.
    ...
    // Creates the selected Sugiyama crossing-minimization module.
    ...
    // Creates the selected Sugiyama coordinate-assignment module.
    ...
    // Creates the selected planar-subgraph heuristic with a safe run count.
    ...
    // Creates the selected edge-insertion module for planarization.
    ...
    // Creates the selected planar embedder used before orthogonal drawing.
    ...
    // Configures the orthogonal drawing model and optional bend bound.
    ...
    // Maps the protocol value to an FMMM stopping criterion.
    ogdf::FMMMOptions::StopCriterion createFmmmStopCriterion(const std::string &id)
    {
        if (id == "threshold")
        {
            return ogdf::FMMMOptions::StopCriterion::Threshold;
        }
        return ogdf::FMMMOptions::StopCriterion::FixedIterations;
    }

    // Maps the protocol value to a stress-minimization termination criterion.
    ogdf::StressMinimization::TerminationCriterion createStressTerminationCriterion(const std::string &id)
    {
        if (id == "position_difference")
        {
            return ogdf::StressMinimization::TerminationCriterion::PositionDifference;
        }
        if (id == "stress")
        {
            return ogdf::StressMinimization::TerminationCriterion::Stress;
        }
        return ogdf::StressMinimization::TerminationCriterion::None;
    }

    // Layout dispatch ------------------------------------------------------------
    // This is the main function that calls the correct OGDF layout implementation.
    void runSelectedLayout(ogdf::GraphAttributes &ga, const LayoutRequest &request)
    {
        ...
        const double threshold = request.threshold > 0.0 ? request.threshold : 0.01;

        // Reject incompatible layouts before clearing bends or changing coordinates.
        if (id == "balloon" && hasDisconnectedComponents(ga.constGraph()))
        {
            warnLayoutNotApplied("BalloonLayout requires a connected graph.");
            return;
        }
        if (id == "tree" && !ogdf::isAcyclicUndirected(ga.constGraph()))
        {
            warnLayoutNotApplied("TreeLayout requires a forest.");
            return;
        }
        if (id == "radial_tree" && !ogdf::isTree(ga.constGraph()))
        {
            warnLayoutNotApplied("RadialTreeLayout requires one connected tree.");
            return;
        }
        if (id == "pivot_mds" && !ogdf::isConnected(ga.constGraph()))
        {
            warnLayoutNotApplied("PivotMDS requires a connected graph.");
            return;
        }
        ...
        else if (id == "fmmm")
        {
            ...
            layout.stopCriterion(createFmmmStopCriterion(request.heuristicOne));
        ...
        else if (id == "stress")
        {
            ...
            layout.convergenceCriterion(createStressTerminationCriterion(request.heuristicOne));
            if (iterations > 0)
            {
    ...
    // Graph generation -----------------------------------------------------------
    // Synthetic graphs are created here, then laid out using either a grid or a default OGDF layout.
    ...
    // Assigns stable IDs, default dimensions, and optional labels to generated nodes.
    ...
    // Creates the graph type selected in the generator request.
    ...
    // Metric calculation and stderr protocol ------------------------------------
    // These helpers emit machine-readable metrics to stderr, which Java reads back from the process.
    // Writes a finite decimal metric as a key-value pair.
    ...
    // Writes an unsigned count metric as a key-value pair.
    void emitMetric(std::ostream &out, const std::string &key, size_t value)
    {
        out << ' ' << key << '=' << value;
    }

    // Writes a signed integer metric as a key-value pair.
    void emitMetric(std::ostream &out, const std::string &key, int value)
    {
        out << ' ' << key << '=' << value;
    }

    // Calculates graph density while safely handling graphs with fewer than two nodes.
    ...
    // Calculates the area of the current drawing's bounding box.
    ...
    // Calculates all supported metrics and emits one machine-readable stderr line.
                     ...
                     const std::string &runtimeMetricKey)
    {
        ...
        // The Java side recognizes this one-line prefix and leaves other stderr text as diagnostics.
...
// Process entry point --------------------------------------------------------
// main() reads the request, chooses layout vs. generation vs. metrics, and writes the result back to VANTED.
int main()
{
    try
    {
        ...
        ogdf::Graph g;
        const long attributes = ogdf::GraphAttributes::nodeGraphics | ogdf::GraphAttributes::edgeGraphics | ogdf::GraphAttributes::nodeLabel | ogdf::GraphAttributes::edgeLabel | ogdf::GraphAttributes::nodeId;
        ogdf::GraphAttributes ga(g, attributes);

        // Generation creates a new graph; layout and metrics first import one.
        ...
        else
        {
            ...
            // Skip layout in metrics-only mode so VANTED's current geometry is measured.
            if (!metricsOnly)
            {
        ...
        // Every successful mode returns a complete graph using the same format.
